...
class Andalu_Woo_Courses_Cart_Reserve {
	...
	public static function quantity_in_carts( $id, $ignore_own_cart = false ) {
		...
		if ( empty( self::$sessions ) ) {
			self::$sessions = $wpdb->get_results(
				"SELECT option_name, option_value FROM $wpdb->options WHERE option_name LIKE '_wc_session_%' AND option_value LIKE '%\"{$field}\";i:{$id};%'"
			);
		}
		...
		if ( ! empty( self::$sessions ) && ! is_wp_error( self::$sessions ) ) {
			...
			foreach ( self::$sessions as $session ) {
				$own_cart = ( $session->option_name === '_wc_session_' . WC()->session->get_customer_id() );
				if ( $ignore_own_cart && $own_cart ) { continue; }
				
				$cart_session = unserialize( $session->option_value );
				if ( isset( $cart_session[ 'cart' ] ) ) {
					...
					foreach ( $cart_items as $item_key => $item ) {
						...
						if ( $id === $item['course_registration'][ $field ] ) {
