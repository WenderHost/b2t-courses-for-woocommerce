...
class Andalu_Woo_Courses_Admin {
	...
	static function woocommerce_wp_select_multiple( $field ) {
	    ...
	    if ( ! empty( $field['description'] ) ) {
	        if ( isset( $field['desc_tip'] ) && false !== $field['desc_tip'] ) {
	            echo '<img class="help_tip" data-tip="' . esc_attr( $field['description'] ) . '" src="' . esc_url( WC()->plugin_url() ) . '/lib/img/help.png" height="16" width="16" />';
	        } else {
		...
		<div id="course_data" class="panel woocommerce_options_panel">
			<div class="options_group">
				<?php
					 ...
					 * Deprecated
					 *
					 * To be removed. These have been replaced by an ACF Repeater Field:
					 */
					woocommerce_wp_text_input( array(
						'id'          => '_course_certification',
						'label'       => __( 'Certification (Deprecated)', 'andalu_woo_courses' ),
					...
					woocommerce_wp_text_input( array(
						...
						'label'       => __( 'Certification Link (Deprecated)', 'andalu_woo_courses' ),
	...
	private static function save_course_classes_data( $post_id, $data ) {
		...
		if ( ! empty( $data['classes'] ) && is_array( $data['classes'] ) ) {
			...
			foreach( $data['classes'] as $class ) {
				...
				$class['price']				= ( empty( $class['price'] ) && is_numeric( $class['price'] ) )? null : $class['price'] ;
		...
		foreach( $classes as $class ) {
			...
			update_post_meta( $class_id, '_price', $class['price'] );
	...
	public static function enqueue_styles_scripts() {
		...
		if ( $is_woocommerce_screen ) {
			...
			wp_enqueue_script( 'andalu_woo_courses_admin', Andalu_Woo_Courses::$url . '/lib/js/course-admin.js', $dependencies, filemtime( plugin_dir_path( __FILE__ ) . '../../lib/js/course-admin.js' ) );
			wp_localize_script( 'andalu_woo_courses_admin', 'Andalu_Woo_Courses', $script_params );

			wp_enqueue_style( 'andalu_woo_courses_admin', Andalu_Woo_Courses::$url . '/lib/css/course-admin.css', array(), '1.0' );
		} // if ( $is_woocommerce_screen )
