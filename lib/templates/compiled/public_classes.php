...
'.LR::sec($cx, (($inary && isset($in['classes'])) ? $in['classes'] : null), null, $in, true, function($cx, $in) {$inary=is_array($in);return '    <div class="row">
      ...
      <div class="details">
        ...
        <div class="time">'.htmlspecialchars((string)(($inary && isset($in['times'])) ? $in['times'] : null), ENT_QUOTES, 'UTF-8').' '.((LR::ifvar($cx, (($inary && isset($in['duration'])) ? $in['duration'] : null), false)) ? '<br/> '.htmlspecialchars((string)(($inary && isset($in['duration'])) ? $in['duration'] : null), ENT_QUOTES, 'UTF-8').'' : '').'</div>
        <div class="price">'.((LR::ifvar($cx, (($inary && isset($in['class_price'])) ? $in['class_price'] : null), false)) ? ' '.htmlspecialchars((string)(($inary && isset($in['class_price'])) ? $in['class_price'] : null), ENT_QUOTES, 'UTF-8').' ' : ' '.((isset($cx['scopes'][count($cx['scopes'])-1]) && is_array($cx['scopes'][count($cx['scopes'])-1]) && isset($cx['scopes'][count($cx['scopes'])-1]['cost'])) ? $cx['scopes'][count($cx['scopes'])-1]['cost'] : null).' ').'</div>
